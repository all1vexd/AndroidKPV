...
import ru.itis.hw_3.domain.model.NotificationPriority.High.priority
    ...
    Column(
        ...
        Button(
            onClick = {
                ...
                if (titleError == null) {
                    val id = if (hasReplyAction) {
                        notificationService.createNotificationWithReply(
                        title = title,
                        message = message,
                    ...
                    Toast.makeText(context, "Уведомление созданно с ID: ${id}", Toast.LENGTH_SHORT).show()
                }
